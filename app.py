...
# רשימת גרגירים מעודכנת עם שמות הקבצים ב-API
berries_data = {
    "Oran": "oran-berry", "Sitrus": "sitrus-berry", "Lum": "lum-berry",
    "Cheri": "cheri-berry", "Chesto": "chesto-berry"
}
...
if menu == "פוקדקס":
    ...
    cols = st.columns(3)
    for i, (name, img) in enumerate(berries_data.items()):
        with cols[i % 3]:
            # הוספת תמונה מ-API
            st.image(f"https://raw.githubusercontent.com/PokeAPI/sprites/master/items/{img}.png", width=100)
            st.subheader(f"{name} Berry")
            if st.button(f"הקש להקראה - {name}", key=name):
                tts = gTTS(text=f"{name} Berry is great for healing", lang='en')
                tts.save("b.mp3")
